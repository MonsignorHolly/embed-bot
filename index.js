...
const embed = new EmbedBuilder()
    ...
    .addFields(
        { name: "📅 Datum", value: `${dateInfo.day}. ${dateInfo.monthName} ${dateInfo.year}`, inline: true },
...
Pokud si nejsi absolutně jistý že jde o závažné porušení, VŽDY vrať violation: false.`}, { role: "user", content:`Zpráva: “${message.content}”\nKanál: ${message.channel.name}`
}
],
max_tokens: 200,
});
...
```
    const embed = new EmbedBuilder()
        ...
        .addFields(
            { name: "👤 Hráč", value: `${message.author} (${message.author.username})`, inline: true },
            { name: "🕐 Čas", value: `<t:${Math.floor(message.createdTimestamp / 1000)}:F>`, inline: true },
            { name: "💬 Dotaz", value: `\`\`\`${message.content.slice(0, 1000)}\`\`\`` }
        )
        .setFooter(FOOTER)
        .setTimestamp();
    ...
    console.error("sendUnknownReport error:", err);
}
```
...
if (interaction.isChatInputCommand()) {
    ...
    if (interaction.commandName === "vyvoj") {
        ...
        if (info) {
            embed.addFields({ name: "ℹ️ Dodatečné informace", value: info });
        }
...
if (message.channel.id === REP_CHANNEL_ID) {
    ...
    await message.delete().catch(() => {});
    await message.channel.send({ embeds: [repEmbed] });
    return;
}
