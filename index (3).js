...
async function sendNameDayMessage() {
    ...
    .addFields(
        ...
        {
            name: "📆 Den", value: dateInfo.dayName, inline: true
        }
    )
    .setFooter(FOOTER)
    .setTimestamp();
    ...
    try {
        const response = await groq.chat.completions.create({
            ...
            messages: [{
                ...
                role: "user", content: `Zpráva: "${message.content}"\nKanál: ${message.channel.name}`
            }],
            max_tokens: 200,
        });
...
async function sendViolationReport(message, violationData) {
    try {
        ...
        { name: "🕐 Čas", value: ` < t: $ {
            Math.floor(message.createdTimestamp / 1000)}: F > `, inline: true },
...
return interaction.reply({
content: "❌ Nemáš oprávnění", ephemeral: true
});
...
.addFields(
{
name: "📋 Co je nového / upraveno / smazáno", value: novinka
}
)
.setTimestamp()
.setFooter({
...
if (info) {
embed.addFields({
name: "ℹ️ Dodatečné informace", value: info
});
}
...
return interaction.reply({
content: "❌ Panel s tímto ID neexistuje", ephemeral: true
});

roleSessions.set(interaction.user.id, {
action: "add", messageId, emoji, channelId: interaction.channel.id
});
...
if (!roles[emoji])
return interaction.reply({
content: `❌ Emoji **${emoji}** není v panelu`, ephemeral: true
});
...
content: "❌ Session vypršela", ephemeral: true
});

const role = interaction.roles.first();
if (!role) return interaction.reply({
content: "❌ Žádná role", ephemeral: true
});
...
chatHistories.clear();
return interaction.reply({
content: "✅ Systémový prompt upraven!", ephemeral: true
});
}
...
return interaction.reply({
content: "✔ Změna uložena, klikni Save pro aplikování", ephemeral: true
});
}
